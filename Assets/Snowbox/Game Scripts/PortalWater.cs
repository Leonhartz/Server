...
public class PortalWater : MonoBehaviour
{
	...
	public Transform m_PortalEntry;
	public Transform m_PortalExit;
	public float m_PortalPlaneOffset = 30;
	...
	public void OnWillRenderObject()
	{
		...
		{
			if (m_PortalEntry == null) m_PortalEntry = transform;
			if (m_PortalExit == null) m_PortalExit = transform;
			
			Transform curCam = cam.transform;
			Transform refCam = refractionCamera.transform;
			
			refCam.position = m_PortalExit.TransformPoint(m_PortalEntry.InverseTransformPoint(curCam.position));
			refCam.rotation = m_PortalExit.rotation * (Quaternion.Inverse(m_PortalEntry.rotation) * curCam.rotation);
			
			// find out the refraction plane: position and normal in world space
			Vector3 normal = m_PortalExit.TransformDirection(m_PortalEntry.InverseTransformDirection(transform.up));
			Vector3 pos = m_PortalExit.position + normal * m_PortalPlaneOffset;
			
			if (new Plane(normal, pos).GetSide(refCam.position))
			{
				// Setup oblique projection matrix so that near plane is our reflection
				// plane. This way we clip everything below/above it for free.
				Vector4 clipPlane = CameraSpacePlane( refractionCamera, pos, normal, -1.0f );
				Matrix4x4 projection = refractionCamera.projectionMatrix;
				CalculateObliqueMatrix (ref projection, clipPlane);
